...
const debugLogger = {
  ...
  error: (message, data = null) => {
    const timestamp = new Date().toISOString().split('T')[1].split('.')[0]
    console.error(`[${timestamp}] [AI-EDITOR] ERROR: ${message}`, data ? data : '')
  },
  success: (message, data = null) => {
    const timestamp = new Date().toISOString().split('T')[1].split('.')[0]
    console.log(`[${timestamp}] [AI-EDITOR] SUCCESS: ${message}`, data ? data : '')
  }
}
...
1. EXPORT FORMAT - MUST use this exact pattern:
   ...
   CORRECT: export default function ComponentName() { return <div>...</div>; }
   WRONG: export default { render: () => ... }
   WRONG: default export function ComponentName() { ... }
   WRONG: export { ComponentName as default }

2. RETURN VALUE - MUST return JSX element:
   CORRECT: return <div>Content</div>;
   CORRECT: return <Component />;
   WRONG: return "string";
...
3. IMPORTS - ONLY React allowed:
   ...
   WRONG: import './styles.css';
   WRONG: import { motion } from 'framer-motion';
   WRONG: import styled from 'styled-components';

4. STATE - ALWAYS initialize with value:
   CORRECT: const [count, setCount] = useState(0);
   CORRECT: const [text, setText] = useState('');
   WRONG: const [count, setCount] = useState();
   WRONG: const [data, setData] = useState(undefined);

5. STYLES - ONLY inline objects:
   CORRECT: const styles = { button: { padding: '10px', color: 'blue' } };
   CORRECT: <div style={{ padding: '10px' }}>Content</div>
   WRONG: import './Button.css';
   WRONG: className="btn-primary"

6. IMAGES - ONLY CORS-enabled URLs:
   CORRECT: https://images.unsplash.com/photo-ID?w=800&h=400&fit=crop
   CORRECT: https://via.placeholder.com/800x400/cccccc/999999?text=Image
   WRONG: https://via.placeholder.com/300x600?text=Image+1
   WRONG: https://picsum.photos/1200/600

7. ASYNC - NEVER in render:
   CORRECT: useEffect(() => { fetch(...).then(...) }, [])
   WRONG: async function Component() { ... }
   WRONG: const data = await fetch(...);
...
function generateBundledHTML(files, entryFile) {
  ...
  for (const [filePath, info] of Object.entries(graph)) {
    ...
    // Escape the file path to prevent issues with special characters
    const escapedPath = filePath.replace(/\\/g, '\\\\').replace(/'/g, "\\'")
    fileDefinitions.push(`  '${escapedPath}': '${encoded}'`)
  }
      ...
      try {
        ...
        function __require(path) {
          ...
          try {
            ...
            compiled = compiled.replace(/require\(\s*['"]([./][^'"]*)['"]\s*\)/g, function(match, importPath) {
        ...
        if (!Component) {
          document.getElementById('root').innerHTML = '<div class="error-display">WARNING: No component found to render.<br><br>Make sure your file exports a default component.</div>';
        } else if (typeof Component !== 'function') {
          document.getElementById('root').innerHTML = '<div class="error-display">WARNING: Invalid component.<br><br>Expected a function or class component, got: ' + typeof Component + '<br><br>Value: ' + JSON.stringify(Object.keys(Component || {})) + '</div>';
        } else {
          try {
            ...
            document.getElementById('root').innerHTML = '<div class="error-display">WARNING: Render Error<br><br>' + String(renderErr.message || renderErr) + '<br><br>' + String(renderErr.stack || '') + '</div>';
          }
        }
        ...
        document.getElementById('root').innerHTML = 
          ...
          '<strong>WARNING: Preview Error</strong><br><br>' +
